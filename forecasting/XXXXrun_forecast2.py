...
def main():
    ...
    try:
        ...
        if not model_row: 
            raise Exception("Belum ada model. Jalankan train_model.py dulu")
        ...
        df["bulan"] = pd.to_datetime(df["bulan"]+"-01",errors="coerce")
        ...
        plt.title(f"Data Aktual + Forecast Produk ID {product_id}")
        plt.xlabel("Bulan")
